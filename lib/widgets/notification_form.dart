...
class NotificationForm extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        Radio<TargetingMode>(
                          value: TargetingMode.segments,
                          groupValue: controller.targetingMode.value,
                          onChanged: (value) =>
                              controller.setTargetingMode(value!),
                        ),
                        const Expanded(
                          child: Text('Send to Audience Segments'),
                        ),
                      ],
                    ),
                    Row(
                      children: [
                        Radio<TargetingMode>(
                          value: TargetingMode.externalIds,
                          groupValue: controller.targetingMode.value,
                          onChanged: (value) =>
                              controller.setTargetingMode(value!),
                        ),
                        const Expanded(
                          child: Text('Send to Specific Users (External IDs)'),
                        ),
                      ],
                    ),
                  ],
          ...
          // Dynamic Audience Selection based on targeting mode
          Obx(() {
            if (controller.targetingMode.value == TargetingMode.segments) {
              return _buildSegmentSelection(controller);
            } else {
              return _buildExternalIdSelection(controller);
            }
          }),
        ],
      ),
  ...
  Widget _buildSegmentSelection(NotificationController controller) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
  ...
  Widget _buildExternalIdSelection(NotificationController controller) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              const Text(
                'Enter external user IDs (comma-separated):',
                style: TextStyle(
              ...
              const SizedBox(height: 8),
              TextFormField(
                controller: controller.externalIdsController,
                maxLines: 3,
                decoration: InputDecoration(
                  hintText: '05012345678,...etc.',
                  helperText: "Enter id's separated by commas",
                ...
                validator: (value) {
                  if (value == null || value.trim().isEmpty) {
                    return 'At least one external user ID is required';
                  }
                  return null;
                },
              ),
              ...
              Obx(() {
                final parsedIds = controller.getParsedExternalIds();
                return Container(
                  padding:
                      const EdgeInsets.symmetric(vertical: 8, horizontal: 12),
                  decoration: BoxDecoration(
                    color: parsedIds.isNotEmpty
                        ? Colors.green.withOpacity(0.1)
                        : Colors.red.withOpacity(0.1),
                    borderRadius: BorderRadius.circular(4),
                  ),
                  child: parsedIds.isNotEmpty
                      ? Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(
                              'Ready to send to ${parsedIds.length} user(s):',
                              style: TextStyle(
                                color: Colors.green[700],
                                fontWeight: FontWeight.bold,
                              ),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'External IDs must match those set in your app for the target users.',
                        style: TextStyle(
                ...
                onPressed: controller.isSending.value
                    ...
                    : () {
                        if (controller.formKeyForContents.currentState!
                            .validate()) {
                          if (!controller.isValidTargeting()) {
                            String message = controller.targetingMode.value ==
                                    TargetingMode.segments
                                ? 'No segments selected. Please select at least one segment.'
                                : 'No external user IDs entered. Please enter at least one user ID.';
                          ...
                          Get.dialog(
                            AlertDialog(
                              ...
                              content: Text(
                                'Send notification to ${controller.getTargetingDescription()}?',
                              ),
                              actions: [
                                TextButton(
